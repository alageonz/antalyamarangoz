...
session_start();
include 'baglan.php';
if (isset($_POST['hakkimizdafotoduzenle'])) {
...
if (isset($_POST['hizmetkaydet'])) {
	...
	if ($insert) {
		...
		Header("Location:../production/hizmet.php?durum=ok");

	} else {

		Header("Location:../production/hizmet.php?durum=no");
	}
...
if (isset($_POST['hizmetduzenle'])) {
	...
	if($_FILES['hizmet_foto']["size"] > 0)  { 
		...
		$duzenle=$db->prepare("UPDATE hizmet SET
			hizmet_ad=:hizmet_ad,
			hizmet_foto=:hizmet_foto	
			WHERE hizmet_id={$_POST['hizmet_id']}");
		$update=$duzenle->execute(array(
			'hizmet_ad' => $_POST['hizmet_ad'],
			'hizmet_foto' => $refimgyol
		));
		...
		if ($update) {
			...
			$resimsilunlink=$_POST['hizmet_foto'];
			unlink("../../$resimsilunlink");

			Header("Location:../production/hizmet-duzenle.php?hizmet_id=$hizmet_id&durum=ok");

		} else {

			Header("Location:../production/hizmet-duzenle.php?durum=no");
		}
...
if (isset($_POST['genelayarkaydet'])) {
	...
	$update=$ayarkaydet->execute(array(
		...
		'ayar_bizisecin' => $_POST['ayar_bizisecin']
